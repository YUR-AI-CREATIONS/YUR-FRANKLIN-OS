...
# from emergentintegrations.llm.chat import LlmChat, UserMessage  # Removed - using local implementation
...
# Simple replacements for removed emergentintegrations classes
class UserMessage:
    def __init__(self, content: str):
        self.content = content

class LlmChat:
    def __init__(self, provider: HybridLLMProvider, system_message: str):
    ...
    async def send_message(self, message: UserMessage) -> str:
        ...
        response = await self.provider.generate(self.system_message, message.content)
...
@supabase_router.post("/auth/signup")
async def signup(request: SignUpRequest):
    """Sign up a new user"""
    from supabase_util import sign_up_user
    try:
        result = await sign_up_user(request.email, request.password, request.metadata)
        return {"success": True, "user": result["user"], "session": result["session"]}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))

@supabase_router.post("/auth/signin")
async def signin(request: SignInRequest):
    """Sign in a user"""
    from supabase_util import sign_in_user
    try:
...
@supabase_router.post("/auth/signout")
async def signout():
    """Sign out current user"""
    from supabase_util import sign_out_user
    await sign_out_user()
    return {"success": True}

@supabase_router.get("/auth/user")
async def get_user():
    """Get current user"""
    from supabase_util import get_current_user
...
@supabase_router.post("/projects")
async def create_project_endpoint(request: CreateProjectRequest):
    """Create a new project"""
    from supabase_util import get_current_user, create_project
    user = get_current_user()
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    
    try:
        project = await create_project(request.name, request.description, user.id, request.metadata)
        return {"success": True, "project": project}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@supabase_router.get("/projects")
async def list_user_projects():
    """List user's projects"""
    from supabase_util import get_current_user, get_user_projects
    user = get_current_user()
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
...
@supabase_router.get("/projects/{project_id}")
async def get_project_endpoint(project_id: str):
    """Get a specific project"""
    from supabase_util import get_project
    try:
...
@supabase_router.put("/projects/{project_id}")
async def update_project_endpoint(project_id: str, request: UpdateProjectRequest):
    """Update a project"""
    from supabase_util import update_project
    try:
...
@supabase_router.delete("/projects/{project_id}")
async def delete_project_endpoint(project_id: str):
    """Delete a project"""
    from supabase_util import delete_project
    try:
...
class StoreCertificationRequest(BaseModel):
    certification: Dict[str, Any]

@supabase_router.post("/projects/{project_id}/certifications")
async def store_certification_endpoint(project_id: str, request: StoreCertificationRequest):
    """Store certification for a project"""
    from supabase_util import store_certification
    try:
        cert = await store_certification(project_id, request.certification)
        return {"success": True, "certification": cert}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@supabase_router.get("/projects/{project_id}/certifications")
async def get_project_certifications_endpoint(project_id: str):
    """Get certifications for a project"""
    from supabase_util import get_project_certifications
    try:
        certifications = await get_project_certifications(project_id)
...
try:
    ...
    logging.warning("Failed to start headless worker: %s", e)
