...
You are GROK - the analytical brain behind FRANKLIN OS. Be direct, technical, and insightful."""
    ...
    def detect_language(self, filename: str, code: str = "") -> str:
        ...
        return 'python'
        """Extract code from Markdown blocks for any language"""
        if not response_text:
            return ""
        
        # Language-specific code block markers
        markers = {
            "python": "```python",
        ...
        marker = markers.get(language.lower(), "```")
        
        if marker in response_text:
            return response_text.split(marker)[1].split("```")[0].strip()
    ...
    async def healer_phase(self, code: str, error: str, mission: str, language: str = "python") -> str:
        """Phase 3: Healer fixes errors for multiple languages"""
        self._log_output("healer", f"Diagnosing and patching {language} code...")
        
        # Language-specific healing prompts
        prompts = {
            "python": f"""
...
""",
            "javascript": f"""
You are the HEALER. The JavaScript code below failed during execution.
...
INSTRUCTIONS:
1. Analyze the error to find the root cause.
2. Fix the logic, syntax, or runtime errors.
3. Return ONLY the full, corrected JavaScript code inside ```javascript blocks.
4. Do not offer explanations, just the code.
""",
            "typescript": f"""
You are the HEALER. The TypeScript code below failed during compilation or execution.
...
INSTRUCTIONS:
1. Analyze the TypeScript compilation or runtime error.
2. Fix type errors, logic issues, or syntax problems.
3. Return ONLY the full, corrected TypeScript code inside ```typescript blocks.
4. Do not offer explanations, just the code.
""",
            "java": f"""
You are the HEALER. The Java code below failed during compilation or execution.
...
INSTRUCTIONS:
1. Analyze the Java compilation or runtime error.
2. Fix syntax, logic, or dependency issues.
3. Return ONLY the full, corrected Java code inside ```java blocks.
4. Do not offer explanations, just the code.
""",
            "csharp": f"""
You are the HEALER. The C# code below failed during compilation or execution.
...
INSTRUCTIONS:
1. Analyze the C# compilation or runtime error.
2. Fix syntax, logic, or .NET issues.
3. Return ONLY the full, corrected C# code inside ```csharp blocks.
4. Do not offer explanations, just the code.
""",
            "go": f"""
You are the HEALER. The Go code below failed during compilation or execution.
...
INSTRUCTIONS:
1. Analyze the Go compilation or runtime error.
2. Fix syntax, logic, or dependency issues.
3. Return ONLY the full, corrected Go code inside ```go blocks.
4. Do not offer explanations, just the code.
""",
            "rust": f"""
You are the HEALER. The Rust code below failed during compilation.
...
INSTRUCTIONS:
1. Analyze the Rust compilation error.
2. Fix ownership, borrowing, type, or syntax issues.
3. Return ONLY the full, corrected Rust code inside ```rust blocks.
4. Do not offer explanations, just the code.
""",
            "php": f"""
You are the HEALER. The PHP code below failed during execution.
...
INSTRUCTIONS:
1. Analyze the PHP runtime error.
2. Fix syntax, logic, or server issues.
3. Return ONLY the full, corrected PHP code inside ```php blocks.
4. Do not offer explanations, just the code.
"""
        }
        
        fix_prompt = prompts.get(language.lower(), prompts["python"])
        
        response = await self.consult_oracle(
            f"You are an expert {language.title()} Debugger and Systems Architect.",
            fix_prompt
        )
        
        if response:
            new_code = self.extract_code(response, language)
            self._log_output("healer", f"Patch generated for {language} ({len(new_code)} chars)")
            return new_code
    ...
    async def genesis_loop(self, mission: str, target_file: str = None) -> GenesisTask:
        ...
        language = self.detect_language(target)
        self._log_output("genesis", f"Detected language: {language}")
        ...
        for attempt in range(1, self.max_retries + 1):
            ...
            if attempt < self.max_retries:
                new_code = await self.healer_phase(task.current_code, result.error, mission, language)
