...
class FileGenerator:
    ...
    def parse_code_blocks(self, llm_output: str) -> List[Dict]:
        ...
        # Pattern 4: Just code blocks with language (infer filename)
        pattern4 = r'```(\w+)\s*\n(.*?)```'
        ...
        # If no files found, try pattern 4 with filename inference
        if not files:
            for i, match in enumerate(re.finditer(pattern4, llm_output, re.DOTALL)):
                ...
                for line in first_lines:
                    # Check for # filename.py or // filename.js comments
    ...
    def create_project_structure(self, build_id: str, files: List[Dict]) -> Tuple[Path, Dict]:
        """
        Create actual project directory with files
